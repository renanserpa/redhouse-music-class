...
export default function MonthlyReportForm({ user, onSave }: Props) {
  ...
  const generateParentReport = async () => {
    ...
    try {
      ...
      const prompt = `
        Você é um coordenador pedagógico musical da RedHouse International School.
        Com base nos dados abaixo do aluno ${student?.name}, gere um parágrafo curto, amigável, profissional e encorajador para ser enviado aos pais.
        
        Regras:
        1. Use um tom acolhedor e focado no progresso.
        2. Evite jargões técnicos excessivos.
        3. Destaque os pontos positivos e dê um toque de entusiasmo sobre os próximos passos.
        4. O texto deve ser em Português do Brasil.
        
        Dados do Aluno (Mês de Referência):
        - Instrumento: ${report.instrument}
        - Nível de Desenvolvimento: ${report.developmentLevel}
        - Destaque do mês: ${report.monthlyHighlight}
        - Conteúdos trabalhados: ${report.workedContent}
        - Pontos de atenção sugeridos: ${report.attentionPoints}
        - Orientações para o próximo mês: ${report.nextMonthGuidance}
      `;
