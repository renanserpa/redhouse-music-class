...
export default function TablatureModule() {
  ...
  const addSection = () => {
    const name = prompt('Nome da Seção:', `Seção ${sections.length + 1}`);
    if (!name) return;
  ...
  const clearTab = () => {
    if (confirm('Deseja limpar toda a tablatura?')) {
      setTabNotes([]);
    }
  };
  ...
  const loadTab = () => {
    ...
    if (saved) {
      ...
      alert('Nenhuma tablatura salva encontrada.');
    }
  };
      ...
      <header className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 glass-card p-3 border-white/5 bg-white/2">
          ...
          <div className="flex gap-2 mr-4 pr-4 border-r border-white/10">
            ...
            <button onClick={clearTab} className="p-3 rounded-xl hover:bg-rose-500/10 text-redhouse-muted hover:text-pedagogy-red transition-all"><Trash2 className="w-5 h-5" /></button>
          </div>
