  ...
  return (
    <div className={`flex items-center justify-between w-full ${isInstitutional ? 'mb-8' : ''}`}>
      <div className="flex items-center gap-4">
        ...
        `}>
          <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[10px] border-r-[10px] border-b-[8px] border-l-transparent border-r-transparent border-b-redhouse-primary"></div>
          <span className="mt-1">RED</span>
          <span>HOUSE</span>
        </div>
        
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <h1 className={`font-black ${isCompact ? 'text-sm' : 'text-2xl'} leading-none tracking-tighter text-redhouse-blue dark:text-white`}>
              REDHOUSE <span className="text-redhouse-primary">MUSIC CLASS</span>
            </h1>
            {!isCompact && (
              <div className="bg-redhouse-primary/10 text-redhouse-primary px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest">
                Bilingual
              </div>
            )}
          </div>
